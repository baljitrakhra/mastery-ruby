...
puts "Enter the number of groups"
no_groups = gets.chomp.to_i
x = 0
y = 0
name = nil
submission = nil
arr_groups = Array.new(no_groups) { Array.new }
while name != "Stop"
  while x != no_groups
    puts "Enter a name"
...
while submission != "Stop"
  array = []
  puts "Enter the number of a group to print out"
  submission = gets.chomp.capitalize
  if submission == "Stop"
    break
  end

  array = arr_groups[submission.to_i - 1]
  print array.join(", ")
  puts
end  
